...
const InviteFriends = () => {
  ...
  const sendEmail = async () => {
    try {
      const response = await axios.post(`http://localhost:1200/api/v1/mail/send-email`, {
        to: email,
        subject,
        //text: `${message}\n\nClick the following link to join the trip: ${generateTripLink(response.data.tripId)}`,
        text: `${message}\n\nClick the following link to join the trip: ${generateTripLink("123456")}`,
      });
  ...
  const generateTripLink = (tripId) => {
    // Generate a unique link for the trip
    const tripLink = `http://localhost:5173/join-trip/${tripId}`;
    return tripLink;
  };
